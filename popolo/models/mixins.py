...
app_config = apps.get_app_config("popolo")
...
class ClassificationShortcutsMixin(object):
    def add_classification(self, scheme, code=None, descr=None, **kwargs):
        ...
        c, created = get_model("Classification").objects.get_or_create(
    ...
    def update_links(self, new_links):
        ...
        for id in new_ids:
            ...
            for u in ul:
                ...
                u_link = u["link"]
                l, created = get_model("Link").objects.update_or_create(
                    url=u_link["url"], defaults={"note": u_link["note"]}
                )
    ...
    def update_sources(self, new_sources):
        ...
        for id in new_ids:
            ...
            for u in ul:
                ...
                u_source = u["source"]
                l, created = get_model("Source").objects.update_or_create(
                    url=u_source["url"], defaults={"note": u_source["note"]}
                )
